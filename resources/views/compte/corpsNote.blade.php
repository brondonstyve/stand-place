...
<div class="col-lg-3 col-md-6">
    <div class="card">
        <div class="card-body">
            ...
            <h5 class="">Travaux Pratiques &nbsp;: <span style="color:@if ($note[$i]->tp<5) red @else @if ($note[$i]->tp<12) blue @else green @endif @endif ;float: right" >{{  $note[$i]->tp }}/20</span> </h5>
                <h5 class="">Controle continu &nbsp;&nbsp;&nbsp;&nbsp;: <span style="color:@if ($note[$i]->CC<5) red @else @if ($note[$i]->CC<12) blue @else green @endif @endif;float: right " >{{ $note[$i]->CC }}/20</span> </h5>
                <h5 class="">Session normale  &nbsp;&nbsp;&nbsp; :<span style="color:@if ($note[$i]->SN<5) red @else @if ($note[$i]->SN<12) blue @else green @endif @endif;float: right" >{{ $note[$i]->SN }}/20</span></h5>
                     @php
                         $comptNote=0;
                         $comptCoef=0;
                         $final=($note[$i]->CC*30/100)+($note[$i]->SN*70/100)
                     @endphp
                <h5 class="">Note finale &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: <span style="color:@if ($final<5) red @else @if ($final<12) blue @else green  @endif @endif ;float: right" >{{ $final }}/20</h5>

            <span class="text-success"> {{ ((($note[$i]->CC*30/100)+($note[$i]->SN*70/100))/20)*100 }}%  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; @if ($final<5) danger @else @if ($final<12) Mauvais travail @else bon travail @endif @endif </span>
            <div class="progress">
                            ...
                            <table class="table m-0 table-colored-full table-full-inverse table-hover btn-sm" style="color: white">
                                ...
                                <tbody>
                                        ...
                                        <tr>
                                                ...
                                                <td style="background-color: white"> <span style="color:@if ($comptNote/$comptCoef<12) red @else green @endif " >{{ $comptNote/$comptCoef }}</span> </td>
                                        </tr>

                                        <tr>
                                                <td colspan="5" style="text-align: right;">décision du conseil des enseignants</td>
                                                <td style="background-color: white"> <span style="color:@if ($comptNote/$comptCoef<12) red @else green @endif " >@if ($comptNote/$comptCoef<12) Echec @else admmis @endif </span> </td>
                                        </tr>
                    ...
                    <div class="table-responsive m-t-20">
                        ...
                        @for ($i = 0; $i < $listec; $i++)
                             <tr class="">
                                    ...
                                    <td><input type="text" size="7" value="@if(sizeOf($remplisseur)==0){{0}}@else @if(sizeOf($remplisseur)>$i){{ $total=($remplisseur[$i]->CC*30/100)+($remplisseur[$i]->SN*70/100) }}@else{{0}}@endif{{ $space }}@endif" disabled></td>
